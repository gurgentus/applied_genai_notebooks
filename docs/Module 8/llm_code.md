...
!!! note "Related Marimo Notebook"
    ...
    - [Module 8 Practical: GPT and Fine-Tuning](https://github.com/gurgentus/applied_genai_notebooks/blob/main/notebooks/Module_8_Practical_GPT.py)

As part of this activity you will update the text generation API developed in Modules 2 and 6 to use a fine-tuned LLM. In particular, you will fine-tune a small GPT2 model (openai-community/gpt2) to handle question-answering. Fine-tuning involves taking a model with pre-trained weights and continuing its training using a new dataset.
...
You will incorporate appropriate code from Module8-GPT notebook into your FastAPI implementation. This involves updating several key steps. 
...
The training loop should be similar to the one in the Module 8-GPT notebook, however, you should remember to use the pretrained model loaded in the previous step instead of the one built from scratch.
